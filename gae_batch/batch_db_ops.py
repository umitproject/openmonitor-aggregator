...
from google.appengine.ext.db import allocate_ids
from common.batch.batch_queries import BatchQuery
...
from google.appengine.api.datastore import Entity, MultiQuery, Put, Get
from google.appengine.api.datastore_errors import Error as GAEError
from google.appengine.api.datastore_types import  Category, Email, Link, \
    PhoneNumber, PostalAddress, Text, Blob, Key
...
class GAEQuery(NonrelQuery):
    ...
    def get_matching_pk(self, low_mark=0, high_mark=None):
        ...
        results = [result for result in datastore.Get(self.pk_filters)
...
class SQLCompiler(NonrelCompiler):
    ...
    def get_instance_data(self, instance, raw=False):
        ...
        for field in instance._meta.local_fields:
            if raw:
                ...
                value = field.pre_save(instance, not instance._entity_exists)
    ...
    def create_entity(self, model, data=tuple()):
        gae_data = {}
        opts = model._meta
        unindexed_fields = get_model_indexes(model)['unindexed']
...
#                    logging.info("Found foreign key in column %s" % column)
    ...
    def create_gae_entity(self, instance, raw=False, allocate_id=False):
        gae_data = {}
        opts = instance.__class__._meta
        unindexed_fields = get_model_indexes(instance.__class__)['unindexed']
        unindexed_cols = [opts.get_field(name).column
                          for name in unindexed_fields]
        kwds = {'unindexed_properties': unindexed_cols}

        data = self.get_instance_data(instance, raw)
    ...
    #                    logging.info("Found foreign key in column %s" % column)
        ...
        if allocate_id:
            handmade_key = db.Key.from_path('video_Post', 1)
            id = allocate_ids(handmade_key, 1)
            if len(id) > 1: id = id[0]
            kwds["id"] = id
...
class SQLInsertCompiler(NonrelInsertCompiler, SQLCompiler):
    ...
    def insert(self, data, return_id=False):
        entity = self.create_entity(self.query.model, data)
    ...
    def batch_save(self, instance_cls_values_raw_list):
        ...
        for instance, cls, values, raw in instance_cls_values_raw_list:
            ...
            for field, value in values:
                ...
                data[field.column] = value
            entity = self.create_entity(cls, data)
            items.append(entity)
            if not cls.__name__ in items_by_type:
                items_by_type[cls.__name__] = 0

            items_by_type[cls.__name__] += 1

        keys = datastore.Put(items, rpc=self._create_rpc())
        pk_vals = [key.id_or_name() for key in keys]

        logging.info(items_by_type)
...
class BatchOperation(object):
    ...
    def get_query(self, instance, using=None):
        ...
        if using not in self.queries:
            ...
            self.queries[using] = BatchQuery(**kwargs)
...
def convert_to_gae_entity(instance, raw=False, allocate_id=False):
    """
    Convenience function to convert a Django entity to GAE entity, without dependency on
    compilers &c.
    """
    using = 'default'
    connection = connections[using]
    compiler = getattr(
            import_module("common.batch.batch_db_ops"), 'SQLCompiler'
        )(None, connection, using)
    return compiler.create_gae_entity(instance, raw, allocate_id)

def generate_id(instance):
    handmade_key = db.Key.from_path(instance.__class__._meta.db_table, 1)
    instance.id = allocate_ids(handmade_key, 1)[0]
